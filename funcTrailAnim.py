...
def place_food(food_str, std, mapsize, mode, mode_input):
    ...
    if mode == 'gothenburg':
        ...
        # g13=np.round(np.array([0.036363636, 0.645454545])*mapsize).astype(int) # To close to others
    ...
    if mode == 'tokyo':
        ...
        #t1=np.round(np.array([0.8125, 0.025])*mapsize).astype(int) # To close to others
        t2=np.round(np.array([0.5,      0.15])*mapsize).astype(int) 
        #t3=np.round(np.array([0.125, 0.15])*mapsize).astype(int) # To close to others
        t4=np.round(np.array([0.1625,   0.15])*mapsize).astype(int)
        t5=np.round(np.array([0.2625,   0.15])*mapsize).astype(int)
        ...
        t15=np.round(np.array([0.85,    0.5])*mapsize).astype(int)
        #t16=np.round(np.array([0.95, 0.4875])*mapsize).astype(int) # To close to others
        ...
        t29=np.round(np.array([0.482,    0.84375])*mapsize).astype(int)
        t30=np.round(np.array([0.375,   0.7675])*mapsize).astype(int)
...
def image_to_matrix(image_path, mapsize):
    ...
    for y in range(img2.shape[0]):
        for x in range(img2.shape[1]):
            ...
            if red == 0 and green == 0 and blue == 0:  # black pixel (sea)
                matrix[y, x] = 1
            else: # white pixel (land)
                matrix[y, x] = 0
